...
my $proc           =
  $project->{'CONFIG'}->{'BWA_PROCESSORS'}; #number of processos for running bwa
my $sleep_time   = 0;
my $bwa_priority = 10;

# making right folder structure
my $script_dir = $project->{'CONFIG'}->{'DIR'} . '/tasks';
$project->make_folder_structure();
##############################

#bwa commands
my $bwa    = $project->{'CONFIG'}->{'BWA'} . "/bwa";
my $mpirun = $project->{'CONFIG'}->{'MPIRUN'} . "/mpirun -np $proc";
...
my $call         = "";
my $genome       = $project->{'CONFIG'}->{'GENOME'};
my $gene_list    = $project->{'CONFIG'}->{'GENELIST'};
...
if ( $tasks{'ALIGN'} ) {
	my @alignes_suffixes;
	for my $lane ( @{ $config->{'DATA'}->{LANE} } ) {
		my $suffix = align_lane($lane);
	}
...
if ( $tasks{'VARIATION'} ) {
	...
	filter_pass( $merged_indels, $sample_id, $indels_passed,
		[$merge_indels_job] );
	...
	my $before_merge = [
		$project->get_job_by_suffix($snps_recal),
		$project->get_job_by_suffix($indels_recal)
	];
	...
	merge_parallel_vcf(
		$project,
		[
			$project->get_out_by_suffix($snps_recal),
			$project->get_out_by_suffix($indels_recal)
		],
		$sample_id,
		$project->get_out_by_suffix($variations),
...
variant_evaluation( $project->get_out_by_suffix($variations),
	$var_stat, $var_evaluation_job,
	[ $project->get_job_by_suffix($variations) ] );
...
sub align_lane {
	my $suffix = "";
	my ($id_forward, $id_reverse, $id_sampe, $id_samse);
	my ($lane) = @_;
	if($lane->{PL} eq 'ILLUMINA'){
		if($lane->{FORWARD}){
			$id_forward = align_fastq( $lane, 'FORWARD' );
		}
		if($lane->{REVERSE}){
			$id_reverse = align_fastq( $lane, 'REVERSE' );
		}		
		if($lane->{FORWARD} && $lane->{REVERSE}){
			$id_sampe = sampe( $lane->{FORWARD}, $lane->{REVERSE} , [$id_forward, $id_reverse]);
		}
		elsif($lane->{FORWARD}){
			$id_samse = samse( $lane->{FORWARD} , [$id_forward]);
		}
		elsif($lane->{REVERSE}){
			$id_samse = samse( $lane->{REVERSE} , [$id_reverse]);
		}
	}
...
sub sampe {
	#$lane->{FORWARD}, $lane->{REVERSE} , [$id_forward, $id_reverse]
	my ( $project, $lane ) = @_;
	sleep($sleep_time);
...
sub align_fastq {
	...
	my $in = $lane->{$type};
	my $out = $project->get_out_by_suffix($suffix);
	my $job_id = $project->get_job_by_suffix($suffix);
	my $program    = "$align -f $out $genome $in";
	my $qsub_param = "-pe mpi $proc -p $bwa_priority";
	$task_scheduler->submit( $job_id, $qsub_param, $program );	
	return $suffix;
...
PROGRAM
	my $qsub_param = '-hold_jid ' . $project->task_id($pre_job_name);
...
PROGRAM
	my $qsub_param =
	  '-hold_jid '
	  . $project->task_id( $project->get_job_by_suffix($snps_tranches) );
...
PROGRAM
	my $qsub_param =
	  '-hold_jid '
	  . $project->task_id( $project->get_job_by_suffix($indels_tranches) );
