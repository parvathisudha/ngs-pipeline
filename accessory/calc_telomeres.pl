...
my $average_repeat_num_per_telomeric_region = $telomere_repeats_num / $telomers_len;


my $telomere_per_chr_num = $average_repeat_num_per_telomeric_region * $tel_per_chr_length;
#my $tel_per_chr_length = $telomers_len / ($coverage * 46);

open RESULT, ">$result_file" or die "Can't write to $result_file\n";
print RESULT "TOTAL_READS\tTELOMERE_REPEATS\tSEQUENCED_LENGTH\tTELOMERE_SEQUENCED_LENGTH\tCOVERAGE\tTELOMERE_PER_CHR_NUM\tTELOMERE_PER_CHR_LENGTH\tTELOMERE_REP_AVERAGE\n";
print RESULT "$total_reads\t$telomere_repeats_num\t$reads_len\t$telomers_len\t$coverage\t$telomere_per_chr_num\t$tel_per_chr_length\t$average_repeat_num_per_telomeric_region\n";
close RESULT;

open DISTRIBUTION, ">$distribution_file" or die "Can't write to $distribution_file\n";
